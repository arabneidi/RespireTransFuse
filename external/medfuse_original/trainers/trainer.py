...
class Trainer():
    ...
    def load_ehr_pheno(self, load_state):
        ...
        checkpoint = torch.load(load_state, map_location='cpu', weights_only=False)
    ...
    def load_state(self):
        if self.args.load_state is None:
            return
        checkpoint = torch.load(self.args.load_state, map_location='cpu', weights_only=False)
    ...
    def load_cxr_pheno(self, load_state):
        ...
        for name, param in state_dict.items():
            ...
            if not name.startswith('cxr_model.'):
                continue
            ...
            if tuple(own_state[name].shape) != tuple(param.shape):
                skipped.append((name, f'{tuple(param.shape)} -> {tuple(own_state[name].shape)}'))
                continue
        ...
        print(f'loaded cxr tensors: {len(loaded)}')
        pass
    ...
    def print_and_write(self, ret, prefix='val', isbest=False, filename='results.txt'):
        ...
        with open(f"{self.args.save_dir}/{filename}", 'a') as results_file:
            if isbest:
                ...
                line = "{} epoch {:03d} | val_auroc={:.5f} | val_auprc={:.5f}\n".format(str(prefix).strip(), int(getattr(self, "epoch", 0)), float(ret["auroc_mean"]), float(ret["auprc_mean"]))
                print(line)
                results_file.write(line)
            else:
                line = "{} epoch {:03d} | val_auroc={:.5f} | val_auprc={:.5f}\n".format(str(prefix).strip(), int(getattr(self, "epoch", 0)), float(ret["auroc_mean"]), float(ret["auprc_mean"]))
