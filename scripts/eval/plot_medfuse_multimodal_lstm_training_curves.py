...
RUN_DIR = BASE / "outputs/medfuse_multimodal_lstm"
HISTORY_PATH = RUN_DIR / "history.csv"
PREDICTIONS_PATH = RUN_DIR / "test_predictions.csv"
SAVE_PATH = (
    RUN_DIR
    / "medfuse_multimodal_lstm_training_validation_calibration.png"
)
...
def load_test_predictions(path):
    ...
    if probability_column is not None:
        ...
        if prediction_column is None:
            raise RuntimeError(
                "No probability or prediction column found. "
                f"Available columns: {frame.columns.tolist()}"
            )
...
figure.suptitle(
    "MedFuse Multimodal LSTM Training, Validation, and Calibration Curves",
