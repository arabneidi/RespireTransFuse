...
def possible_leakage_flag(variable, label, source):
    ...
    IMPORTANT:
        Do NOT include the source string in the searched text.
        Source values such as "chartevents" and "labevents" contain the substring "vent",
        which would falsely mark every feature as leakage.

    Search only the human variable/label text.
    """
    import re
    ...
    # Remove the table prefix so "chartevents" / "labevents" cannot trigger "vent".
    # Example:
    #   chartevents::220045::Heart Rate::valuenum -> heart rate
    parts = variable_text.split("::")
    if len(parts) >= 3:
        clean_variable_text = parts[2]
    else:
        clean_variable_text = variable_text

    text = f"{clean_variable_text} {label_text}".lower()

    # Regex patterns with boundaries where needed.
    leakage_patterns = [
        # respiratory support / ventilation settings
        r"\bventilator\b",
        r"\bventilation\b",
        r"\bvent mode\b",
        r"\bpeep\b",
        ...
        r"respiratory rate \(total\)",
        r"respiratory rate \(spontaneous\)",
        r"\bapnea\b",
        r"\bintub",
        r"\bextub",
        r"\bett\b",
        ...
        # invasive monitoring / high-intensity proxy
        r"\bpa line\b",
        r"\bpulmonary artery\b",
        r"\baortic pressure\b",
        r"\bpvr\b",
        r"\bsvr\b",
        r"\bsignal\b",
        r"\bcco\b",
    ...
    return any(re.search(pattern, text) for pattern in leakage_patterns)
...
def clinical_category(variable, label, source):
    ...
    if source == "chartevents":
        if any(k in text for k in ["fio2", "inspired o2", "fraction inspired oxygen", "oxygen flow", "o2 flow"]):
            return "respiratory_support_intensity"
...
def elasticnet_stability_selection(
    ...
    for b in range(n_bootstraps):
        ...
        if (b + 1) % 10 == 0:
            print(f"Finished bootstrap {b + 1}/{n_bootstraps}; successful models={model_count}", flush=True)
...
def main():
    ...
    summary = {
        ...
        "outputs": {
            ...
            "top_consensus_preview": str(top_preview_path) if "top_preview_path" in locals() else None,
        },
    }
